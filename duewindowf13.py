...
with open("Names, Due Dates, and Lengths F13.csv", "rU") as f:
    ...
    for row in reader:
        ...
        #row[4] = time.strptime(row[4].split('+')[0], "%Y-%m-%dT%H:%M:%S.%f") 
        #dueDates [row[0]] = time.mktime(row[4])
        dueDates [row[0]] = row[4]

for name, date in dueDates.iteritems():
	date = time.strptime(date, "%Y-%m-%dT%H:%M:%S.%f")
...
for line in liblytics.read_log_file("tracking_700x_UMass__Fall_2013.log.gz"): #Reads line in log file
	if (line["event_type"] == "play_video"):  # Grabs only play_videos
		...
		if username != "": 
			...
			if videoName not in usersDict and times > dueDate[videoName]: #second loop creates third dict
				videoDict = {}
				videoDict["Times"] = []
...
# Prints to CSV
#f = open("Window (number) min f13.csv", "w") #This is where you could name the CSV
#f.write("username,")
#for cleanName in sorted(videoNames):
#	f.write(cleanName)
#	f.write(",")
